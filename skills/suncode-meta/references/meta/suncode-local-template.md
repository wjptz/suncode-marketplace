# Suncode Local Skill Template

Copy this template to create a project-specific `suncode-local` skill.

---

## How to Use

1. Create directory: `mkdir -p .claude/skills/suncode-local`
2. Copy the template below to `.claude/skills/suncode-local/SKILL.md`
...
description: |
  ...
  This skill documents all modifications made to the vanilla Suncode system.
  Inherits from suncode-meta for base architecture documentation.
  Use this skill to understand what's been customized in this project's Suncode setup.
---
...
This skill documents all customizations made to Suncode in this project. For vanilla Suncode documentation, see the `suncode-meta` skill.

## Base Information

| Field            | Value      |
| ---------------- | ---------- |
| Suncode Version  | X.X.X      |
...
#### /suncode:command-name [ALL]
- **File**: `.claude/commands/suncode/command-name.md`
...
#### Category Name
- **Path**: `.suncode/spec/category-name/`
- **Files**: [list of files]
...
#### spec-name.md
- **Path**: `.suncode/spec/category/spec-name.md`
- **Change**: [what was changed]
- **Date**: YYYY-MM-DD
- **Reason**: [why it was changed]
-->
...
Document any special steps needed when upgrading Suncode.

<!-- Template:
### Upgrade to Suncode X.Y.Z
...
```bash
#!/bin/bash
# create-suncode-local.sh

PROJECT_NAME="${1:-$(basename $(pwd))}"
SKILL_DIR=".claude/skills/suncode-local"
...
name: suncode-local
description: |
  Project-specific Suncode customizations for PROJECT_NAME_PLACEHOLDER.
  This skill documents all modifications made to the vanilla Suncode system.
  Inherits from suncode-meta for base architecture documentation.
---

# Suncode Local - PROJECT_NAME_PLACEHOLDER

## Base Information

| Field | Value |
|-------|-------|
| Suncode Version | $(cat package.json 2>/dev/null | grep version | head -1 | cut -d'"' -f4 || echo "unknown") |
