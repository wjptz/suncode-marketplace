...
Complete guide to `.suncode/worktree.yaml` configuration.
...
**Note**: Suncode uses a custom YAML parser (not PyYAML). Supports basic key-value pairs and arrays; complex nested structures may not work.
...
Suncode uses a custom YAML parser (not PyYAML) with these limitations:
