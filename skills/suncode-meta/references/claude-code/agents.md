...
Documentation for the Suncode agent system - specialized AI agents for different development phases.

---

## Overview

Suncode uses **specialized agents** for different tasks. Each agent has specific capabilities, restrictions, and context injection.
...
```
1. Read .suncode/.current-task → find task directory
...
inject-subagent-context.py runs
            │
            ├── Read .suncode/.current-task
            │
            ├── Find task directory
            │
            ├── Load implement.jsonl
            │   {"file": ".suncode/spec/backend/index.md", "reason": "..."}
...
### Task Workflow (from /suncode:start)
