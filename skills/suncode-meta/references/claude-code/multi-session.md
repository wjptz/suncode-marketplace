...
│                                                                             │
│  /suncode:parallel → Configure task → start.py                             │
│                                           │                                 │
│                                           │ Creates worktree               │
│                                           │ Starts agent                   │
│                                           ▼                                 │
...
Location: `.suncode/worktree.yaml`
...
```bash
python3 .suncode/scripts/multi_agent/start.py <task-dir>
```
...
```bash
python3 .suncode/scripts/multi_agent/start.py .suncode/tasks/01-31-add-login-taosu
# Output: Started agent in ../suncode-worktrees/feature/add-login
```
...
```bash
# Overview
python3 .suncode/scripts/multi_agent/status.py

# Detailed view
python3 .suncode/scripts/multi_agent/status.py --detail <task-name>

# Watch mode
python3 .suncode/scripts/multi_agent/status.py --watch <task-name>

# View logs
python3 .suncode/scripts/multi_agent/status.py --log <task-name>

# Show registry
python3 .suncode/scripts/multi_agent/status.py --registry
```
...
Resume stopped sessions:
  cd ../suncode-worktrees/feature/fix-api && claude --resume <session-id>
```
...
```bash
python3 .suncode/scripts/multi_agent/create_pr.py [--dry-run]
```

**Actions**:

1. Stage changes: `git add -A`
2. Exclude: `git reset .suncode/workspace/`
...
```bash
# Specific worktree
python3 .suncode/scripts/multi_agent/cleanup.py <branch-name>

# All merged worktrees
python3 .suncode/scripts/multi_agent/cleanup.py --merged

# All worktrees (with confirmation)
python3 .suncode/scripts/multi_agent/cleanup.py --all
```

**Actions**:

1. Archive task to `.suncode/tasks/archive/YYYY-MM/`
...
**Location**: `.suncode/workspace/<developer>/.agents/registry.json`
  ...
  "agents": [
    {
      ...
      "worktree_path": "/abs/path/to/suncode-worktrees/feature/add-login",
      "pid": 12345,
      "started_at": "2026-01-31T10:30:00",
      "task_dir": ".suncode/tasks/01-31-add-login-taosu"
    }
  ]
}
```
...
```bash
# Create task
python3 .suncode/scripts/task.py create "Add login" --slug add-login

# Configure
python3 .suncode/scripts/task.py init-context <task-dir> fullstack
python3 .suncode/scripts/task.py set-branch <task-dir> feature/add-login
...
```bash
python3 .suncode/scripts/multi_agent/status.py --watch add-login
```
...
# Cleanup
python3 .suncode/scripts/multi_agent/cleanup.py feature/add-login
```
...
```bash
# Session 1
python3 .suncode/scripts/multi_agent/start.py .suncode/tasks/01-31-add-login

# Session 2 (immediately)
python3 .suncode/scripts/multi_agent/start.py .suncode/tasks/01-31-fix-api

# Session 3
python3 .suncode/scripts/multi_agent/start.py .suncode/tasks/01-31-update-docs
...
# Resume
cd ../suncode-worktrees/feature/task-name
...
**State** (`.suncode/.ralph-state.json`):
...
4. Check `.suncode/.ralph-state.json`
...
# Manual edit
vim .suncode/workspace/<dev>/.agents/registry.json
```
